...
CREATE TABLE IF NOT EXISTS `user_uc_org_links` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `idx_uc_org_type_id` (`org_type`, `org_id`),
    KEY `idx_uc_p_root_id` (`uc_p_root_id`)
...
CREATE TABLE IF NOT EXISTS `user_uc_member_links` (
    ...
    `profile_json`     JSON         NULL COMMENT 'userList 全量详情（用户同意后）',
    ...
    `updated_at`       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    KEY `idx_uc_member_org_link` (`org_link_id`),
    KEY `idx_uc_member_user` (`user_id`),
    KEY `idx_uc_member_p_stu` (`p_stu_id`)

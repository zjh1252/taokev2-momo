...
import {
  getEAApplicationDetail,
  approveEAApplication,
  rejectEAApplication
} from '@/features/enterprise-agents/api/service';
import { eaKeys } from '@/features/enterprise-agents/api/queries';
...
export default function EnterpriseAgentApplicationDetailPage(props: PageProps) {
  ...
  const fetchDetail = async () => {
    try {
      const res = await getEAApplicationDetail(uid);
      setDetail(res.data);
    } catch {
        ...
        <div className='flex items-center gap-2'>
          {isPending ? (
            <InlineAuditActions
              showApprove
              showReject
              subjectLabel={detail.applicantName || detail.nickname || '该经纪公司'}
              approveTitle='确认通过'
              approveDescription={`确定要通过 ${detail.applicantName || '该经纪公司'} 的入驻/资料变更申请吗？`}
              rejectTitle='驳回申请'
              rejectDescription='请填写驳回原因，申请人将收到通知。'
              onApprove={async () => {
                await approveEAApplication(detail.userId);
                toast.success('已通过');
                void fetchDetail();
              }}
              onReject={async (reason) => {
                await rejectEAApplication(detail.userId, reason);
                toast.success('已驳回');
                void fetchDetail();
              }}

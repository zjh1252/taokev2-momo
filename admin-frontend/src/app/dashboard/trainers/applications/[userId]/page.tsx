...
import {
  ...
  approveApplication,
  rejectApplication
} from '@/features/trainers/api/service';
...
export default function TrainerApplicationDetailPage(props: PageProps) {
  ...
  const fetchDetail = async () => {
    try {
      const res = await getTrainerApplicationDetail(uid);
      setDetail(res.data);
    } catch {
        ...
        <div className='flex items-center gap-2'>
          {isPending ? (
            <InlineAuditActions
              showApprove
              showReject
              subjectLabel={detail.applicantName || detail.nickname || '该专家'}
              approveTitle='确认通过'
              approveDescription={`确定要通过 ${detail.applicantName || '该专家'} 的入驻/资料变更申请吗？`}
              rejectTitle='驳回申请'

...
import {
  openCourseEnrollmentKeys,
  openCourseEnrollmentsQueryOptions,
} from '../../api/queries';
import { exportOpenCourseEnrollments } from '../../api/service';
import { columns } from './columns';

export function OpenCourseEnrollmentsTable() {
  ...
  const { data: resp } = useSuspenseQuery(
    openCourseEnrollmentsQueryOptions(filters),
  );
  ...
  const handleExport = async (mode: 'selected' | 'all') => {
    try {
      if (mode === 'selected') {
        ...
        await exportOpenCourseEnrollments({ ids: selectedIds });
      } else {
        await exportOpenCourseEnrollments({ filters });
      }
      toast.success('导出成功');
    } catch {
      toast.error('导出失败');
...
export function OpenCourseEnrollmentsTableSkeleton() {
  return (

...
export const columns: ColumnDef<AdminEnterpriseAgentApplication>[] = [
  ...
  { id: 'status', accessorKey: 'status', header: '状态', enableColumnFilter: true, cell: ({ cell, row }) => { if (row.original.reapplying) { return <Badge variant='secondary'>重提申请</Badge>; } const status = cell.getValue<number>(); return <Badge variant={statusVariant(status)}>{APPLICATION_STATUS_MAP[status] ?? '未知'}</Badge>; }, meta: { label: '状态', variant: 'select' as const, options: APPLICATION_STATUS_OPTIONS } },

...
export const columns: ColumnDef<AdminEnterpriseAgentApplication>[] = [
  { accessorKey: 'id', header: '申请ID', enableSorting: false },
  { accessorKey: 'enterpriseAgentId', header: '经纪公司ID', enableSorting: false, cell: ({ cell }) => cell.getValue<number | null>() ?? '-' },
  ...
  { id: 'status', accessorKey: 'status', header: '状态', enableColumnFilter: true, cell: ({ cell, row }) => { const status = cell.getValue<number>(); return (<div className='flex items-center gap-1'><Badge variant={statusVariant(status)}>{APPLICATION_STATUS_MAP[status] ?? '未知'}</Badge>{row.original.reapplying ? <Badge variant='secondary'>资料重审</Badge> : null}</div>); }, meta: { label: '状态', variant: 'select' as const, options: APPLICATION_STATUS_OPTIONS } },
  ...
  { id: 'submittedAt', header: '提交时间', cell: ({ row }) => { const val = row.original.updatedAt || row.original.createdAt; if (!val) return '-'; return new Date(val).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }); } },

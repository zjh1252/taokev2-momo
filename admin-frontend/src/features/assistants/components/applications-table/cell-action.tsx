...
import type { AdminAssistantApplication } from '../../api/types';
...
interface CellActionProps {
  data: AdminAssistantApplication;
}
...
export function CellAction({ data }: CellActionProps) {
  ...
  const approveMutation = useMutation({
    mutationFn: () => approveAssistantApplication(data.userId),
    onSuccess: () => {
      toast.success('审核通过');
  ...
  const rejectMutation = useMutation({
    mutationFn: () => rejectAssistantApplication(data.userId, reason),
    onSuccess: () => {
      toast.success('已驳回');
      setRejectOpen(false);
      setReason('');
  ...
  return (
    <>
      <AlertModal
        ...
        title='确认通过'
        description={`确定要通过 ${data.nickname || '该用户'} 的助理入驻申请吗？`}
      />

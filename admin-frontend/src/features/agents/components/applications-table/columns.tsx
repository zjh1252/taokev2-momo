...
function statusVariant(status: number) {
  switch (status) { case 1: return 'default'; case 2: return 'secondary'; case 3: return 'destructive'; default: return 'outline'; }
}

export const columns: ColumnDef<AdminAgentApplication>[] = [
  { accessorKey: 'id', header: 'ID', enableSorting: false },
  { id: 'applicant', header: '申请人', cell: ({ row }) => (<div className='flex flex-col'><span className='font-medium'>{row.original.nickname || '-'}</span>{row.original.phone && (<span className='text-muted-foreground text-xs'>{row.original.phone}</span>)}</div>) },
  { accessorKey: 'bio', header: '服务介绍', cell: ({ cell }) => { const val = cell.getValue<string>(); return val ? (val.length > 30 ? val.slice(0, 30) + '...' : val) : '-'; } },

...
export function CellAction({ data }: CellActionProps) {
  ...
  const approveMutation = useMutation({
    mutationFn: () => approveInstitutionApplication(data.userId),
    onSuccess: () => {
      toast.success('审核通过');
  ...
  const rejectMutation = useMutation({
    mutationFn: () => rejectInstitutionApplication(data.userId, reason),
    onSuccess: () => {
      toast.success('已驳回');
      setRejectOpen(false);
      setReason('');
  ...
  return (
    <>
      <AlertModal
        ...
        title='确认通过'
        description={`确定要通过 ${data.orgName || data.nickname || '该用户'} 的机构入驻申请吗？`}
      />

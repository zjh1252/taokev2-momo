...
export function CellAction({ data }: CellActionProps) {
  ...
  const approveMutation = useMutation({
    mutationFn: () => approveTrainerHighlight(data.id),
    onSuccess: () => {
      toast.success('审核通过');
      setApproveOpen(false);
      void queryClient.invalidateQueries({
        queryKey: trainerHighlightKeys.all
      });
    },
  ...
  const rejectMutation = useMutation({
    mutationFn: () => rejectTrainerHighlight(data.id, reason),
    onSuccess: () => {
      toast.success('已驳回');
      setRejectOpen(false);
      setReason('');
  ...
  return (
    <>
      <AlertModal
        ...
        title='确认通过'
        description={`确定要通过「${data.title || '该精彩瞬间'}」的审核吗？`}
      />

      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className='sm:max-w-md'>
          <DialogHeader>
            <DialogTitle>驳回精彩瞬间</DialogTitle>
            <DialogDescription>
              请填写驳回原因，提交者将收到通知。
            </DialogDescription>
          </DialogHeader>

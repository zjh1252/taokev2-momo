...
import { allianceAmbassadorApplicationDetailQueryOptions } from '../../api/queries';
import { ALLIANCE_AMBASSADOR_STATUS_MAP } from '../../api/types';
...
export function ApplicationDetailDialog({
  ...
  const { data, isPending, isError } = useQuery({
    ...allianceAmbassadorApplicationDetailQueryOptions(applicationId),
    enabled: open
  });
  ...
  const fields = application
    ? [
        ...
        ['大使编号', application.ambassadorCode],
        ['协议版本', application.agreementVersion],
        ['审核状态', ALLIANCE_AMBASSADOR_STATUS_MAP[application.status]],
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-2xl'>
        <DialogHeader>
          <DialogTitle>推广大使申请详情</DialogTitle>

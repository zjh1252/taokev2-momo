...
import { CrawlSourceFormDialog } from './crawl-source-form-dialog';
import type {
  CrawlJob,
  CrawlSource,
  CrawledCourse,
...
function pageList<T>(payload?: PagePayload<T> | null) {
  return {
    list: payload?.list ?? [],
...
function CrawlQueryError({ title, error }: { title: string; error: Error | null }) {
  return (
    <Card>
      <CardContent className='space-y-3 py-10 text-center'>
        <p className='font-medium text-destructive'>{title}</p>
        <p className='text-sm text-muted-foreground'>{error?.message || '请稍后重试'}</p>
        <div className='text-muted-foreground mx-auto max-w-xl text-left text-xs leading-6'>
          <p>请依次确认：</p>
          <ol className='list-decimal pl-5'>
            <li>Java 后端已重新编译并启动（本地 dev 端口为 18080，含 /admin/crawl/*）</li>
            <li>数据库已执行 Flyway 迁移 V105–V108（含 crawl_sources 表）</li>
            <li>当前登录账号拥有 SUPER_ADMIN 权限</li>
            <li>Python 爬虫服务已启动（默认 `http://localhost:8100`）</li>
          </ol>
        </div>
      </CardContent>
    </Card>
...
function isCrawlSourceEnabled(source: CrawlSource) {
...
function hasCrawlSourceId(source: CrawlSource): source is CrawlSource & { id: number } {
  return typeof source.id === 'number' && source.id > 0;
}
...
export function CrawlSourcesPanel() {
  const { data: sources = [], isLoading, isError, error } = useQuery(sourcesQueryOptions());
  ...
  const legacyMode =
    !isLoading && sources.length > 0 && sources.every((source) => !hasCrawlSourceId(source));

  if (isError) {
    return <CrawlQueryError title='数据源加载失败' error={error} />;
  }

  const openCreate = () => {
    if (legacyMode) {
      toast.error('请先重新编译并重启 Java 后端，再新增数据源');
      return;
    }
  ...
  const openEdit = (source: CrawlSource) => {
    if (!hasCrawlSourceId(source)) {
      toast.error('当前后端未返回数据源 ID，请执行 mvn install -pl taoke-app -am -DskipTests 后重启 Java');
      return;
    }
  ...
  const handleDelete = (source: CrawlSource) => {
    if (!hasCrawlSourceId(source)) {
      toast.error('当前后端未返回数据源 ID，无法删除');
      return;
    }
    if (source.builtIn) {
      toast.error('内置数据源不可删除，可改为禁用');
      return;
    }
    if (!window.confirm(`确定删除数据源「${source.name} / ${source.dataType}」？`)) {
      return;
    }
  ...
  const handleTrigger = (source: CrawlSource) => {
    if (!isCrawlSourceEnabled(source)) {
      toast.error('该数据源已禁用');
      return;
    }
    ...
    <div className='space-y-4'>
      {legacyMode ? (
        <Alert variant='destructive'>
          <AlertTitle>后端版本过旧，无法编辑数据源</AlertTitle>
          <AlertDescription>
            当前接口未返回数据源 id（仍为硬编码列表）。请在 backend 目录执行
            {' '}
            <code>mvn install -pl taoke-app -am -DskipTests</code>
            {' '}
            并重启 Java，确保 Flyway V108（crawl_sources 表）已执行后再编辑/启用。
          </AlertDescription>
        </Alert>
      ) : null}
      <Card>
        <CardHeader className='flex flex-row items-center justify-between space-y-0'>
          ...
          <Button size='sm' onClick={openCreate} disabled={legacyMode}>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {isLoading && <LoadingRow key='sources-loading' colSpan={6} />}
              {!isLoading && sources.length === 0 && (
                <EmptyRow key='sources-empty' colSpan={6} text='暂无数据源' />
              )}
              {sources.map((source) => (
                <TableRow key={source.id ?? `${source.code}-${source.dataType}`}>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge variant={isCrawlSourceEnabled(source) ? 'outline' : 'secondary'}>
                      {isCrawlSourceEnabled(source) ? 'AVAILABLE' : 'DISABLED'}
                    </Badge>
                    {source.builtIn ? (
                      <div className='mt-1 text-xs text-muted-foreground'>内置</div>
                    ) : null}
                    {source.remark ? (
                    ...
                    <div className='flex justify-end gap-2'>
                      ...
                      {!source.builtIn ? (
                        <Button
                          ...
                          disabled={deleteMutation.isPending || legacyMode}
                        >
                          删除
                        </Button>
                      ) : null}
                      <Button
                        ...
                        disabled={triggerMutation.isPending || !isCrawlSourceEnabled(source)}
                      >
...
export function CrawlJobsPanel() {
  ...
  const query = useQuery({
    ...
    refetchInterval: (query) => {
      const jobs = pageList<CrawlJob>(query.state.data).list;
  ...
  if (query.isError) {
    return <CrawlQueryError title='任务列表加载失败' error={query.error} />;
  }

  const { list: jobs } = pageList<CrawlJob>(query.data);
...
export function CrawledTrainersPanel() {
  ...
  if (query.isError) {
    return <CrawlQueryError title='爬取专家列表加载失败' error={query.error} />;
  }

  const { list: trainers, total } = pageList<CrawledTrainer>(query.data);
...
export function CrawledCoursesPanel() {
  ...
  if (query.isError) {
    return <CrawlQueryError title='爬取课程列表加载失败' error={query.error} />;
  }

  const { list: courses, total } = pageList<CrawledCourse>(query.data);

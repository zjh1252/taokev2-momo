...
erDiagram
    ...
    permissions ||--o{ role_permissions : "被角色关联"
    roles ||--o{ user_role_assignments : "分配给用户"
    users ||--o{ user_role_assignments : "拥有角色"
    users ||--o{ system_operation_logs : "执行操作"
    ...
    confirmation_action_configs {
        int id PK
        varchar action_code UK
        varchar action_name
        varchar module
        tinyint requires_confirmation
        varchar confirmation_message_template
        datetime created_at
        datetime updated_at
    }
```
...
| 旧表 | 新表 | 说明 |
|------|------|------|
| `tk_admingroups` | `roles` + `role_permissions` | 旧表以 `adminid` 和 `adminright` 文本字段存储权限，新系统拆分为独立的角色表和权限关联表，支持细粒度 RBAC |
| `tk_adminlogs` | `system_operation_logs` | 旧表 `action` 为文本描述，新表结构化拆分为 `module` + `action` + `target_type` + `target_id`，支持精准查询与审计；旧 `logdate` (int) 迁移为 `datetime` |
| `tk_adminonlines` | Redis 会话管理 | 旧表用数据库追踪管理员在线状态，新系统使用 Redis 管理后台会话，不再单独建表 |
| `tk_configs` | `system_configs` | 旧表为简单 `ckey`-`cvalue` 键值对，新表增加 `config_group`（分组）、`value_type`（类型校验）、`is_editable`（只读保护）等字段，功能显著增强 |
| `tk_commonlogs` | `user_operation_logs`（用户模块） + `system_operation_logs` | 旧表前台日志迁入用户模块的操作日志表；后台管理日志迁入本模块的系统操作日志表 |
| `tk_action_config` | `permissions` + `confirmation_action_configs` | 旧表 `actname` + `listtips`/`detailstips` 拆分为权限节点定义与二次确认配置两张表，职责更清晰 |

### 7.2 旧权限模型到新 RBAC 的迁移

| 旧模型特点 | 新模型改进 |
|-----------|-----------|
| `tk_admingroups.adminright` 存储逗号分隔的权限字符串 | 权限节点独立建表 `permissions`，通过 `role_permissions` 关联角色，支持标准 RBAC |
| `tk_admingroups.adminviewright` 控制菜单可见性 | 前端菜单渲染基于权限编码动态控制，无需单独存储菜单权限 |
| `tk_admingroups.viewAll` 控制数据范围 | 数据范围控制通过角色配合代码逻辑实现，不在权限表中维护 |
| 权限管理与管理员组耦合 | 角色与权限完全解耦，支持自定义角色与灵活权限组合 |

### 7.3 关键变更点

1. **RBAC 体系重建**：旧系统以简单的管理员分组 + 权限字符串实现权限控制，扩展性差。新系统采用标准 RBAC 模型（角色-权限独立建表），支持自定义角色、细粒度权限节点、权限树层级管理
2. **系统配置结构化**：旧 `tk_configs` 为无分类的键值对，新 `system_configs` 增加分组、类型校验、只读保护，且配置变更全量审计
3. **第三方接口统一管理**：旧系统无集中化的第三方接口配置，新系统通过 `third_party_configs` 统一管理所有外部服务，加密存储配置数据，并增加健康检查与自动重试能力
4. **操作日志结构化**：旧 `tk_adminlogs` 的 `action` 为自由文本，新 `system_operation_logs` 结构化为 `module` + `action` + `target_type` + `target_id`，支持精准检索与统计
5. **备份/恢复能力**：旧系统无系统级备份管理能力，新系统新增 `system_backups` 表，支持全量/增量备份、远程安全存储、一键恢复
6. **二次确认机制**：旧系统无关键操作确认机制，新系统通过 `confirmation_action_configs` 可配置哪些操作需要二次确认，提升运营安全性
7. **时间字段规范化**：旧系统使用 `int` 时间戳（`logdate`），新系统统一使用 `datetime`
8. **管理员在线追踪方式变更**：旧 `tk_adminonlines` 通过数据库表追踪在线状态，新系统利用 Redis 管理会话，更高效且不占用数据库资源

### 7.4 数据迁移注意事项

- 旧 `tk_admingroups.adminright` 权限字符串需逐一解析，映射到新 `permissions` 表的权限编码，再建立 `role_permissions` 关联
- 旧 `tk_adminlogs.logdate` 为 `int` 时间戳，迁移时需转换为 `datetime`
- 旧 `tk_adminlogs` 中的 `action` 自由文本需做关键词提取，映射到新表的 `module` + `action` 结构化字段
- 旧 `tk_configs` 的所有配置项需归类到 `config_group`，并确定 `value_type`
- 旧 `tk_commonlogs` 需按日志性质拆分：前台用户操作 → `user_operation_logs`，后台管理操作 → `system_operation_logs`
- 迁移完成后需验证所有后台管理员的角色/权限映射正确，确保无权限丢失或越权

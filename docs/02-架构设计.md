...
### 3.3 taoke-supply — 讲师 + 经纪人 + 机构

| 业务 PRD | 核心职责 |
|----------|----------|
| trainer.prd.md | 讲师入驻审核、资质认证、个人主页、授课案例 |
| agent.prd.md | 经纪人入驻、讲师资源库管理、资料文件上传 |
| organization.prd.md | 机构入驻审核、师资团队管理、机构资质 |

三者共享"入驻审核"的业务模式，合为一个模块降低管理成本。对外暴露 `TrainerService`、`OrganizationService` 等接口。
...
graph TD
    ...
    SUPPLY[taoke-supply<br/>讲师+经纪人+机构]
...
- `taoke-course` 需要查询讲师信息 → 调用 `taoke-supply` 的 `TrainerService.getById()`
...
- 评价审核通过 → `taoke-content` 发布 `ReviewApprovedEvent` → `taoke-supply` 监听后更新讲师平均评分

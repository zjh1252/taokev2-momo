...
| 指标 | 改前（基线 :3000 / 旧码） | 改后（worktree :3002） | Δ |
|------|---------------------------|-------------------------|---|
| 首页热 TTFB avg | **1.842s** | **0.547s** | **-1.295s（约 -70%）** |
| 专家列表热 TTFB avg | **0.499s** | **0.526s** | **+0.027s（约 +5%，本轮未改列表路径，噪声级）** |
| 专家详情热 TTFB avg | **0.415s** | **0.461s** | **+0.046s（约 +11%，本轮未改详情路径，噪声级）** |
| 首页热 application-code | ≈ **1.74–1.88s** | ≈ **0.40–0.47s**（稳定段） | 约 **-1.3s** |
| 同次 spot：旧 :3000 vs 新 :3002 | 2.256s | 0.507s | **-1.749s** |
...
### 功能冒烟（curl HTTP + HTML 标记）

验收补测（worktree `:3002` 短暂重启后 `curl.exe`）：

| 路径 | HTTP | 热 TTFB（单次） |
|------|------|-----------------|
| `GET http://localhost:3002/` | **200** | 0.518s |
| `GET http://localhost:3002/trainers` | **200** | 0.481s |
| `GET http://localhost:3002/trainers/56185` | **200** | 0.446s |

首页 HTML（≈ 366KB）标记检索：

| 区块 | 检索标记 | 命中 |
|------|----------|------|
| 轮播 / Hero | `首页轮播图` / `轮播` / `banner` | **1** / **1** / **59** |
| 城市频道 | `城市公开课频道` / `热门城市` / `/city/` | **2** / **3** / **47**（字面「城市频道」**0**——组件标题为「城市公开课频道」） |
| 其它区块 | `专家` / `内训` / `公开课` / `案例` | **39** / **26** / **21** / **21** |
| 运营位内容 | `关明生` / `张帆` / `办公室人员` | **4** / **6** / **6** |

## 验收清单

- [x] 至少一个样本页有可对比数字改善：首页热 TTFB **1.842s → 0.547s（Δ -1.295s）**
- [x] 列表/详情亦记录数值 Δ（未改路径，接近噪声）：列表 **0.499→0.526s（+0.027s）**；详情 **0.415→0.461s（+0.046s）**
- [x] 每项代码修复有证据对应：
...
- [x] 功能冒烟：三页 curl 均为 **HTTP 200**；首页 HTML 含轮播（`首页轮播图`）、城市频道（`城市公开课频道`/`热门城市`）、专家/内训/公开课/案例及运营位内容

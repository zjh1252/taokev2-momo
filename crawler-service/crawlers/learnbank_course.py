...
当前 www.learnbank.com.cn 能解析到业务 IP，但 HTTP/HTTPS 端口不可达；learnbank.com.cn 裸域名无业务 A 记录。
适配器只做可访问性诊断，不编造公开课、线上课或内训课数据。
"""
import asyncio
import logging
import re
...
BASE_URL = "http://www.learnbank.com.cn"
ALTERNATE_URLS = (
    "http://www.learnbank.com.cn",
    "https://www.learnbank.com.cn",
    "http://learnbank.com.cn",
    "https://learnbank.com.cn",
)
...
def fetch_text(url: str, timeout: int = 5, retries: int = 1) -> str:
...
def classify_probe_error(exc: Exception) -> Dict[str, Any]:
    ...
    elif "timed out" in text or "WinError 10060" in text:
        reason = "source_timeout"
    elif "actively refused" in text or "WinError 10061" in text:
        reason = "tcp_connection_refused"
...
def coverage_notes() -> list[str]:
    return [
        "OPEN_OFFLINE 未覆盖：www.learnbank.com.cn 当前 HTTP/HTTPS 端口不可达，未能访问公开课课表、城市、价格或地址字段。",
        "OPEN_ONLINE 未覆盖：www.learnbank.com.cn 当前 HTTP/HTTPS 端口不可达，未能访问线上公开课、直播课或报名入口。",
        "INTERNAL 未覆盖：www.learnbank.com.cn 当前 HTTP/HTTPS 端口不可达，未能访问内训、企业定制或方案入口。",
        "当前实现保留可运行适配器和不可达状态识别，避免在源站不可访问时编造课程数据。",
    ]


def iter_learnbank_courses(max_items: int | None = None):
    status = probe_source()
    if not status.get("collectable"):
        logger.warning("learnbank source is not collectable: reason=%s status=%s", status.get("reason"), status)
        return
    logger.warning("learnbank source unexpectedly has course signals, but no parser is enabled yet")
    return
    yield  # pragma: no cover


def crawl_learnbank_courses(max_items: int | None = None) -> List[Dict[str, Any]]:
    return list(iter_learnbank_courses(max_items))
...
class LearnbankCourseSpider:
    """Learnbank 课程爬虫适配器，供 JobManager 调用。"""

    name = "learnbank_course"
    source = "learnbank"
    data_type = "COURSE"
    max_items = None
    supported_course_types: tuple[str, ...] = ()
    coverage_note = (
        "www.learnbank.com.cn 当前能解析到 211.144.39.43，但 80/443 端口不可达；learnbank.com.cn 裸域名无业务 A 记录。"
        "未发现可采集的公开课、线上课或内训课入口，适配器只识别不可采集状态并返回空结果。"
    )
    ...
    async def stream(self) -> AsyncGenerator[Dict[str, Any], None]:
        iterator = iter_learnbank_courses(self.max_items)
        while True:

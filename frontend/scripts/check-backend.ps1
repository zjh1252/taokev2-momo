# Check backend API before starting Next.js dev server
# Usage:
#   .\scripts\check-backend.ps1           # strict (exit 1 if down)
#   .\scripts\check-backend.ps1 -WarnOnly # predev: warn but allow npm run dev
...
foreach ($base in $candidates) {
    ...
    try {
        ...
        if ($resp.StatusCode -eq 200) {
            ...
            $configured = Normalize-ApiBase (Read-DotEnvValue $envFile "NEXT_PUBLIC_API_BASE_URL")
            if ($configured -and $configured -ne $base) {
                Write-Host "TIP: .env.local has $configured but $base works — consider updating NEXT_PUBLIC_API_BASE_URL"
            }
            exit 0
        }
...
Write-Host "  2. Use shared dev server in frontend/.env.local:"
Write-Host "       NEXT_PUBLIC_API_BASE_URL=http://10.0.14.20:8080"

...
import type { PxbCourseListConfig } from './config/pxb-course-list-config';
...
function formatTrainerLocation(course: CourseListItem): string {
  const parts = [course.trainerProvinceName, course.trainerCityName]
    .filter((s) => s?.trim())
    .map((s) => s!.trim());
...
export function PxbCourseListItem({ course, config }: Props) {
  ...
  const href = `${config.basePath}/${course.id}.htm?origin=91pxb`;
    ...
    <div className="pxb-course-item">
      ...
      <div className="pxb-course-body">
        {config.listLayout === 'open' ? (
          ...
          <div className="pxb-course-grid">
            <div className="pxb-grid-cell w260">授课讲师：{course.trainerName || '待定'}</div>
            <div className="pxb-grid-cell w290">讲师所在地：{formatTrainerLocation(course)}</div>
            <div className="pxb-grid-cell w160">课程时长：{hours || 0}小时</div>
            <div className="pxb-grid-cell w260">
              课程价格：{price > 0 ? `￥${price}元` : '待定'}
            </div>
            <div className="pxb-grid-cell w290">课程分类：{course.categoryName || '-'}</div>
          </div>
        )}

...
export default function BuyerWorkCertPage() {
  const [loading, setLoading] = useState(true);
  const [records, setRecords] = useState<BuyerWorkCert[]>([]);
  const [editing, setEditing] = useState<BuyerWorkCert | null>(null);
  const [showForm, setShowForm] = useState(false);

  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await listBuyerWorkCerts();
  ...
  const handleEdit = (r: BuyerWorkCert) => {
    setEditing({ ...r });
    setShowForm(true);
  };
...
function WorkForm({ record, onCancel, onSaved }: FormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && record.id) {
        await updateBuyerWorkCert(record.id, payload);
        toast.success('已更新，重新进入待审核');
      } else {
        await createBuyerWorkCert(payload);

  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="page__scroll" :style="{ paddingTop: navBarH + 'px' }">
      <view class="page__inner">
        ...
        <view class="section">
          <view class="section__head">
            <text class="section__title">已加入的机构</text>
          </view>
          <TkLoading v-if="loading" />
          <TkEmpty v-else-if="!active.length" text="暂未加入任何机构" />
          <view v-else class="list">
            <view v-for="item in active" :key="item.id" class="card">
              <text class="card__name">{{ item.counterpartOrgName || displayName(item) }}</text>
              <text class="card__meta">确认时间：{{ fmtDate(item.confirmedAt) }}</text>
              <view class="card__actions">
                <view class="card__act card__act--danger" @tap="handleUnbind(item)"><text>离开机构</text></view>
              </view>
            </view>
          </view>
        </view>
...
function displayName(item) {
  return item.counterpartOrgName || item.counterpartNickname || `机构#${item.counterpartUserId}`;
}
...
async function handleConfirm(item) {
  ...
  uni.showToast({ title: '已加入机构', icon: 'none' });
  await load();
}
...
async function submitReject() {
  ...
  await rejectBindingByEmployee(rejectItem.value.id, rejectReason.value.trim() || undefined);
...
.page__inner { padding: $tk-sp-3; display: flex; flex-direction: column; gap: $tk-sp-4; }
.section {

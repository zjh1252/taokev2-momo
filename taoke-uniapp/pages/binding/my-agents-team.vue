...
<template>
  <view class="page">
    <TkNavBar title="我的团队" left-icon="back" />
    ...
    <scroll-view scroll-y class="page__scroll" :style="{ paddingTop: navBarH + 'px' }">
      <view class="page__inner">
        ...
        <TkEmpty v-else-if="!filtered.length" text="暂无团队成员" />
        <view v-else class="list">
          <view v-for="item in filtered" :key="item.id" class="card">
            ...
            <text v-if="item.rejectReason" class="card__reject">驳回：{{ item.rejectReason }}</text>
            ...
            <view v-else-if="canUnbind(item)" class="card__actions">
              <view class="card__act card__act--danger" @tap="handleUnbind(item)"><text>解除绑定</text></view>
            </view>
          </view>
        </view>
      </view>
...
const statusTabs = [
  { key: 'all', label: '全部' },
  { key: 'pending', label: '待审核', predicate: (b) => b.status === BINDING_STATUS.PENDING && !b.ifInitiator },
...
async function load() {
  loading.value = true;
  try {
    items.value = await listEnterpriseAgentMembers() || [];
  } catch (_) {
    items.value = [];
  } finally {
    loading.value = false;
...
function displayName(item) {
  return item.counterpartNickname || item.counterpartRealName || `经纪人#${item.counterpartUserId}`;
}
...
async function submitReject() {
  ...
  await rejectAgentByEnterprise(rejectItem.value.id, rejectReason.value.trim() || undefined);
...
.tabs { white-space: nowrap; margin-bottom: 8rpx; }
.tab {
...
.card {
  ...
  &__reject { font-size: $tk-fs-xs; color: #DC2626; margin-top: 4rpx; }

<!--
  专家工作认证 — 多记录列表 + 新增/编辑表单
-->
<template>
  <view class="page">
    <TkNavBar :title="showForm ? formTitle : '工作认证'" :left-icon="showForm ? '' : 'back'" />

    <scroll-view scroll-y class="page__scroll" :style="{ paddingTop: navBarH + 'px' }">
      <view class="page__inner">
        <text v-if="!showForm" class="desc">填写您的工作经历，可添加多条记录。每条记录单独审核。</text>

        <view v-if="showForm && editing" class="form-panel">
          <text class="form-panel__title">{{ editing.id ? '编辑工作记录' : '新增工作记录' }}</text>

          <view class="field">
            <text class="field__label">单位名称 <text class="req">*</text></text>
            <input class="field__input" :value="form.companyName" @input="setField('companyName', $event)" />
          </view>
          <view class="field">
            <text class="field__label">担任职务 <text class="req">*</text></text>
            <input class="field__input" :value="form.position" @input="setField('position', $event)" />
          </view>
          <view class="field">
            <text class="field__label">起始时间 <text class="req">*</text></text>
            <picker mode="date" :value="form.startDate" @change="form.startDate = $event.detail.value">
              <view class="field__picker">{{ form.startDate || '请选择起始时间' }}</view>
            </picker>
          </view>
          <view class="field">
          ...
          <view class="field">
            <text class="field__label">工作描述</text>
            <textarea
              class="field__textarea"
              :value="form.jobDescription"
              placeholder="可选填，简述该岗位的核心职责或成就"
              placeholder-style="color:#999"
              @input="setField('jobDescription', $event)"
            />
          </view>
          <view class="field field--upload">
            <text class="field__label">工作证明文件 <text class="req">*</text></text>
            <TkCertFileUpload v-model="form.proofFile" label="工作证明" camera-only />
          </view>
        ...
        <template v-else>
          ...
          <view v-else-if="!records.length" class="empty">
            <text class="empty__txt">暂无工作认证记录</text>
            <view class="empty__btn" @tap="handleNew">
              <text class="empty__btn-txt">新增工作经历</text>
            </view>
          </view>
            ...
            <view v-for="(r, idx) in records" :key="r.id" class="row" @tap="handleEdit(r)">
              ...
              <view class="row__main">
                <view class="row__title-row">
                  <text class="row__company">{{ r.companyName }}</text>
                  <text class="row__position">{{ r.position }}</text>
                  <TkCertStatusBadge :status="r.status" />
                </view>
                <text class="row__meta">{{ r.startDate }} ~ {{ r.endDate || '至今' }}</text>
          ...
          <view v-if="records.length" class="fab" @tap="handleNew">
            <text class="fab__txt">新增工作经历</text>
          </view>
        </template>
      </view>
    </scroll-view>
...
const formTitle = computed(() => (editing.value?.id ? '编辑工作' : '新增工作'));
...
async function fetchData() {
  loading.value = true;
  try {
    records.value = await certApi.listWorkCerts() || [];
  } catch (_) {
    records.value = [];
  } finally {
    loading.value = false;
...
function resetForm(record) {
  ...
  form.jobDescription = record?.jobDescription || '';
...
async function handleSave() {
  ...
  if (!form.companyName.trim()) return uni.showToast({ title: '请输入单位名称', icon: 'none' });
  if (!form.position.trim()) return uni.showToast({ title: '请输入担任职务', icon: 'none' });
  if (!form.startDate) return uni.showToast({ title: '请选择起始时间', icon: 'none' });
  if (form.endDate && form.endDate < form.startDate) {
    return uni.showToast({ title: '结束日期不能早于开始日期', icon: 'none' });
  }
  if (!form.proofFile) return uni.showToast({ title: '请上传工作证明文件', icon: 'none' });
  ...
  try {
    if (editing.value?.id) {
      await certApi.updateWorkCert(editing.value.id, payload);
      uni.showToast({ title: '已更新，重新进入待审核', icon: 'none' });
    } else {
      await certApi.createWorkCert(payload);
...
function handleDelete(id) {
  uni.showModal({
    ...
    success: async (res) => {
      ...
      await certApi.deleteWorkCert(id);

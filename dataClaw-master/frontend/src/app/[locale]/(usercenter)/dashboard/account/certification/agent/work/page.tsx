 ...
 */
export default function AgentWorkCertPage() {
  const [loading, setLoading] = useState(true);
  const [records, setRecords] = useState<AgentWorkCert[]>([]);
  const [editing, setEditing] = useState<AgentWorkCert | null>(null);
  const [showForm, setShowForm] = useState(false);

  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await listAgentWorkCerts();
  ...
  const handleEdit = (r: AgentWorkCert) => {
    setEditing({ ...r });
    setShowForm(true);
  };
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="text-sm text-gray-500 mt-2">
            填写您作为经纪人的工作经历，可添加多条记录。每条记录单独审核。
          </div>
        </div>
        {!showForm && (
          <button
...
function WorkForm({ record, onCancel, onSaved }: FormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && record.id) {
        await updateAgentWorkCert(record.id, payload);
        toast.success('已更新，重新进入待审核');
      } else {
        await createAgentWorkCert(payload);

...
**数据爬取功能 — 实施计划编写 + 框架代码搭建**

- 编写完整实施计划，保存至 `docs/参考文档/数据爬取功能实施计划.md`
- 架构决策：Python FastAPI 爬虫服务独立部署（:8100），通过 REST API + Webhook 与 Java 后端（:8080）通信，爬取数据先进中间表待审核再入库
- 试点目标：讲师宝 (jiangshi99.com) 专家数据 + 企学宝 (lmschina.net) 课程数据
- 已创建的文件：
  - 数据库迁移：V64（crawled_trainers）、V65（crawled_courses）、V66（crawl_jobs）
  - Java Entity/Repository/DTO/Controller/Service：CrawledTrainer、CrawledCourse、CrawlJob 全套（taoke-admin 模块）
  - Python 爬虫服务骨架：FastAPI + Scrapling，含 base.py、jiangshibao_trainer.py、lmschina_course.py、job_manager.py、callback_service.py
  - 前端 API 层：types.ts、service.ts、queries.ts、mutations.ts、server-service.ts
  - 前端页面占位：trainers/courses/jobs/sources 四个 page.tsx
  - 导航配置：nav-config.ts 新增"数据爬取"分组
- 待完成：CSS 选择器需手动验证、导入逻辑 TODO、前端 UI 组件待实现

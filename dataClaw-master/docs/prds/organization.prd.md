...
| 要素 | 说明 |
|------|------|
| 成员身份 | 机构下可配置多个业务员成员，成员均为独立 `users` 账号 |
| 账号登录 | 业务员使用个人账号登录，按机构成员身份访问机构业务能力 |
| 可执行操作 | 可执行讲师团队维护、课程运营相关的日常操作，并可发起机构关联经纪人的新增与维护 |
| 权限边界 | 业务员不允许修改机构基础信息（机构名称、机构简介、联系方式、资质信息、主页核心展示信息） |
| 管理范围 | 业务员仅能在所属机构范围内操作，不可跨机构管理数据 |
...
| 角色 | 权限说明 |
|------|----------|
| 机构管理员 | 机构入驻、主页管理、师资管理、课程管理、收益查看、公开课状态管理的完整操作权限；可管理机构业务员成员权限与分工 |
| 机构业务员 | 机构成员账号，具备受限业务操作权限：可维护讲师团队、课程运营、发起机构关联经纪人；不可修改机构基础信息与资质信息 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/organizations/{id}/members` | 获取机构成员列表（管理员可见） |
| POST | `/api/v1/organizations/{id}/members` | 新增机构业务员成员（管理员操作） |
| PUT | `/api/v1/organizations/{orgId}/members/{memberId}/status` | 启用/停用机构业务员成员（管理员操作） |
| POST | `/api/v1/organizations/{id}/agents` | 发起机构关联经纪人（管理员/业务员可操作，按权限校验） |
...
1. 机构支持多个业务员成员，成员均为独立用户账号，需先加入机构后方可执行机构侧业务操作
2. 业务员可执行：讲师团队日常维护、课程/公开课运营相关操作、机构关联经纪人新增与维护
3. 业务员不可执行：机构基础信息编辑、资质信息编辑、机构主页核心展示信息修改
4. 管理员可根据业务需要启用/停用业务员成员账号的机构操作权限
5. 所有成员关键操作需记录操作日志（操作者、操作对象、时间、结果）

...
| 职责 | 说明 |
|------|------|
| 订单管理 | 支持在线课、线下公开课的在线下单与订单全生命周期管理 |
| 多方式支付 | 企业用户支持公对公转账、企业微信/支付宝支付；个人用户支持微信/支付宝个人支付 |
...
erDiagram
    users ||--o{ orders : "用户下单"
    orders ||--o{ order_items : "包含明细"
    orders ||--o{ order_attendees : "参课人"
    orders ||--o{ payments : "支付记录"
    orders ||--o{ refunds : "退款记录"
    orders ||--o{ invoices : "发票记录"
    orders ||--o{ order_operation_logs : "操作日志"
    users ||--o{ withdrawals : "提现申请"
    order_items ||--o{ order_attendees : "明细参课人"
    ...
    order_items {
        int id PK "主键"
        int order_id FK "关联 orders.id"
        int course_id "课程ID"
        varchar course_type "课程类型"
        varchar title "课程名称(快照)"
        int trainer_id "讲师ID"
        varchar trainer_name "讲师姓名(快照)"
        int org_id "机构ID"
        varchar org_name "机构名称(快照)"
    ...
    order_attendees {
        int id PK "主键"
        int order_id FK "关联 orders.id"
        int order_item_id FK "关联 order_items.id"
        varchar attendee_name "参课人姓名"
        varchar attendee_phone "参课人手机号"
        tinyint check_in_status "签到状态"
    }
    ...
    payments {
        ...
        decimal amount "支付金额"
        tinyint status "支付状态"
        varchar channel_trade_no "渠道交易号"
    ...
    refunds {
        ...
        int payment_id "关联支付记录"
        decimal amount "退款金额"
        varchar reason "退款原因"
        tinyint status "退款状态"
        varchar refund_method "退款方式"
        varchar channel_refund_no "渠道退款号"
        int reviewer_id "审核人"
    ...
    invoices {
        ...
        decimal amount "开票金额"
        tinyint status "发票状态"
        varchar invoice_file_url "电子发票URL"
        datetime issued_at "开票时间"
    }

    withdrawals {
        int id PK "主键"
        varchar withdrawal_no UK "提现单号"
        int user_id FK "关联 users.id"
        varchar user_role "申请人角色"
        decimal amount "提现金额"
    ...
    order_operation_logs {
        ...
        varchar action "操作类型"
        varchar action_detail "操作详情"
        tinyint before_status "操作前状态"
        tinyint after_status "操作后状态"
...
| 旧表 | 新表 | 说明 |
|------|------|------|
| `tk_course_order` | `orders` | 旧订单主表迁入新订单主表，新增订单类型、折扣、凭证等字段 |
| `tk_course_order_course` | `order_items` | 旧订单课程明细，新增讲师/机构快照信息、折扣明细 |
| `tk_course_order_pay` | `payments` | 旧支付信息表，新增支付方式枚举、渠道交易号、状态流转 |
| `tk_course_order_alipay` | `payments` (method=ALIPAY) | 旧支付宝账户信息合并至统一支付记录表 |
| `tk_course_order_bank` | `payments` (method=BANK_TRANSFER) | 旧银行账户信息合并至统一支付记录表 |
| `tk_course_order_invoice` | `invoices` | 旧发票表，新增发票类型（普票/专票）、税号、接收邮箱、电子发票文件等字段 |
| `tk_course_order_refund` | `refunds` | 旧退款表，新增退款单号、退款方式、渠道退款号、审核流程 |
| `tk_course_order_operate` | `order_operation_logs` | 旧操作记录表，新增操作前后状态快照、操作人角色 |
| `tk_course_order_current_status` | `orders.status` | 旧表通过关联表跟踪状态，新系统直接在订单主表维护当前状态 |
| `tk_course_order_pay2company` | `withdrawals` + 结算模块 | 旧付款给机构记录拆分为提现申请 + 结算管理 |
| `tk_cashout_apply` | `withdrawals` | 旧提现申请表，新增提现单号、角色区分、审核流程 |

### 7.2 关键字段对照

```
tk_course_order.order_code           → orders.order_no
tk_course_order.createtime           → orders.created_at（int → datetime）
tk_course_order.message              → orders.remark

tk_course_order_course.course_title  → order_items.title
tk_course_order_course.course_price  → order_items.unit_price
tk_course_order_course.course_num    → order_items.quantity
tk_course_order_course.course_userid → order_items.org_id（机构 ID）
tk_course_order_course.course_company → order_items.org_name

tk_course_order_pay.pay_type         → payments.method（1=支付宝→ALIPAY, 2=线下→BANK_TRANSFER）
tk_course_order_pay.trade_code       → payments.channel_trade_no
tk_course_order_pay.total            → payments.amount
tk_course_order_pay.userid           → orders.user_id（上移至订单主表）

tk_course_order_invoice.invoice_type → invoices.type（重新定义枚举）
tk_course_order_invoice.invoice_amount → invoices.amount

tk_course_order_refund.refund_amount → refunds.amount
tk_course_order_refund.reason        → refunds.reason
tk_course_order_refund.is_agree      → refunds.status（重新定义状态机）
tk_course_order_refund.is_over       → refunds.status（合并至统一状态）

tk_course_order_operate.type_id      → order_operation_logs.action（改为字符串枚举）
tk_course_order_operate.status       → order_operation_logs.after_status

tk_cashout_apply.userid              → withdrawals.user_id
tk_cashout_apply.amount              → withdrawals.amount
tk_cashout_apply.bank                → withdrawals.bank_name
tk_cashout_apply.bank_account        → withdrawals.bank_account
tk_cashout_apply.isapprove           → withdrawals.status（-1/0/1 → 0/1/2/3 四状态）
```

### 7.3 关键变更点

1. **订单结构规范化**：旧系统支付信息分散在多张表（`alipay`、`bank`、`pay`），新系统统一为 `payments` 表，通过 `method` 字段区分支付方式
2. **状态机简化**：旧系统通过 `tk_course_order_current_status` 关联表追踪状态且涉及位运算（`all_status` 字段），新系统在订单主表直接维护状态字段，清晰可读
3. **订单类型扩展**：新增 `order_type` 字段区分在线课/公开课/版权课/内训课，旧系统无此区分
4. **折扣体系**：新增充值折扣、团购折扣支持，旧系统仅有简单佣金字段（`opencourse_discount`）
5. **发票能力升级**：旧系统发票仅区分"淘课代开/机构自开"，新系统支持增值税普票/专票、电子发票发送
6. **退款流程完善**：旧系统退款字段分散（`is_agree`/`is_over`/`is_accept`），新系统统一为 `status` 状态机 + 审核流程
7. **提现管理升级**：旧 `tk_cashout_apply` 字段简陋，新系统增加提现单号、角色区分、完整审核与结算流转
8. **时间字段规范化**：旧系统使用 `int` 时间戳（`createtime`），新系统统一使用 `datetime`
9. **新增功能**：订单凭证（PDF）生成与下载、公开课参课人管理（`order_attendees`）、组合支付、操作日志前后状态快照

### 7.4 数据迁移注意事项

- 旧 `tk_course_order_pay.pay_type` 值 `1=支付宝` 映射为 `ALIPAY`，`2=线下支付` 映射为 `BANK_TRANSFER`
- 旧系统订单状态（1 待确认 → 8 已取消）需按新状态机映射：1→1, 2→2, 3→2, 4→2, 5→3, 8→4, 9→4
- 旧 `tk_course_order_course.all_status` 位运算字段无需迁移，新系统通过独立的操作日志追溯
- 旧 `tk_cashout_apply.isapprove` 三值（-1/0/1）映射为新系统四状态（0=待审核→0, 1=通过→2, -1=未通过→3）
- 金额字段从 `float` 升级为 `decimal(12,2)`，迁移时注意精度转换
- 旧 `createtime`（int 时间戳）统一转换为 `datetime` 格式

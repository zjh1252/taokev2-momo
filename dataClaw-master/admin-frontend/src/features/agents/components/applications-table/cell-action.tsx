...
import type { AdminAgentApplication } from '../../api/types';
import { Icons } from '@/components/icons';
import { useState } from 'react';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'sonner';
import { approveAgentApplication, rejectAgentApplication } from '../../api/service';
import { agentKeys } from '../../api/queries';

interface CellActionProps { data: AdminAgentApplication; }
...
export function CellAction({ data }: CellActionProps) {
  ...
  const approveMutation = useMutation({ mutationFn: () => approveAgentApplication(data.userId), onSuccess: () => { toast.success('审核通过'); setApproveOpen(false); void queryClient.invalidateQueries({ queryKey: agentKeys.all }); }, onError: () => toast.error('操作失败') });
  const rejectMutation = useMutation({ mutationFn: () => rejectAgentApplication(data.userId, reason), onSuccess: () => { toast.success('已驳回'); setRejectOpen(false); setReason(''); void queryClient.invalidateQueries({ queryKey: agentKeys.all }); }, onError: () => toast.error('操作失败') });

  return (
    <>
      <AlertModal isOpen={approveOpen} onClose={() => setApproveOpen(false)} onConfirm={() => approveMutation.mutate()} loading={approveMutation.isPending} title='确认通过' description={`确定要通过 ${data.nickname || '该用户'} 的经纪人入驻申请吗？`} />

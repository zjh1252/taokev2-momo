...
interface CellActionProps {
  data: AdminInstitutionEmployeeApplication;
}
...
export function CellAction({ data }: CellActionProps) {
  ...
  const approveMutation = useMutation({
    mutationFn: () => approveInstEmployeeApplication(data.userId),
    onSuccess: () => {
      toast.success('审核通过');
      setApproveOpen(false);
      void queryClient.invalidateQueries({ queryKey: instEmployeeKeys.all });
    },
    onError: () => toast.error('操作失败')
  });

  const rejectMutation = useMutation({
    mutationFn: () => rejectInstEmployeeApplication(data.userId, reason),
    onSuccess: () => {
      toast.success('已驳回');
      setRejectOpen(false);
      setReason('');
  ...
  return (
    <>
      <AlertModal
        ...
        title='确认通过'
        description={`确定要通过 ${data.nickname || '该用户'} 的机构员工入驻申请吗？`}
      />
